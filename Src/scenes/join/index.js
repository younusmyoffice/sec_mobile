...
const{appid}=routes?.params || {};
 ...
 console.log("app id",routes?.params?.appid)
  ...
  const checkAppointmentValidity = async () => {
    ...
    try {
      const response = await axios.get(
        `${baseUrl}patient/getAppointmentDateTime/${appid}`,
      );
  ...
  const checkSocketId = async () => {
    try {
      const response = await axios.get(
        `${baseUrl}patient/getUpdateSocketId/${appid}`,
      );
  ...
  const update_socketID = async (meetingId) => {
    try {
      const response = await axiosInstance.put(`patient/putSocketId/`, {
        appointment_id: appid, // Using appointment state as fallback for params?.appID
        socket_id: meetingId,
      });
