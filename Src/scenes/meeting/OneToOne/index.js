...
export default function OneToOneMeetingViewer() {
  ...
  async function updateAudioDeviceList() {
    ...
    try {
      console.log('🔍 [MOBILE] Fetching audio devices in meeting...');
      ...
      console.log('✅ [MOBILE] Audio devices fetched in meeting:', {
      ...
      console.error('❌ [MOBILE] ERROR fetching audio devices in meeting:', error);
  ...
  useEffect(() => {
    if (Platform.OS == "ios") {
      ...
      return () => {
        // BUG FIX: Use removeAllListeners instead of removeSubscription (method doesn't exist)
        // This matches the implementation in ConferenceMeetingViewer
        if (VideosdkRPK && typeof VideosdkRPK.removeAllListeners === 'function') {
